...
(html/deftemplate ingredients-template "listings.html"
  [site-title language-image language-link ingredients-grouped ingredient-name-path categories site-name title-text]
...
(html/defsnippet recipe-list-item "listings.html"
  ...
  [name-path {:keys [id] :as recipe}]
  [:li :a] (html/do->
            (html/set-attr :href (format "recipe?Id=%s" id))
            (html/content (get-in recipe name-path))))

(html/deftemplate recipes-template "listings.html"
  [site-title language-image language-link recipes-grouped recipe-name-path categories site-name title-text]
  [:div#recipes :h1] (html/content site-title)
  [:div#language-logo :img] (html/set-attr :src language-image)
  [:div#language-logo :a] (html/set-attr :href language-link)
  [:div#ingredients-list] (html/content
...
(html/defsnippet thumbnail "index.html" [[:div.thumbnail (html/nth-of-type 1)]]
  [thumbnail-link name description recipe-link link]
  [:img] (html/set-attr :src thumbnail-link)
  [:h2] (html/content name)
  [:p] (html/content description)
  [:a.info] (html/content recipe-link)
  [:a] (html/set-attr :href (format "recipe?Id=%s" link)))

(html/deftemplate main-template "index.html"
  [recipe-link recipes categories max-ingredients site-title site-name up-image language-image language-link  title-text]
  [:div#thumbnails] (html/content
                     (map (fn [{:keys [title ingredients thumbnail-link id]}]
                            (thumbnail thumbnail-link title (render-list 7 ingredients) recipe-link id))
                          recipes))
  [:ul#menu] (html/content
              (map (fn [{:keys [title link]}]
                     (menu-bar title (name link)))
...
(html/deftemplate recipe-template "recipe.html"
  [{:keys [date title images serving time origin] :as recipe} ingredients language
